...
| :-------: | :-------------------------------------------------------------------------------: | :-------------------------------------------------------------: |
|   Adam    | $\quad\|A\|_{\text{max}} = \|vec(A)\|_\infty = \max_{i,j}\|A_{i,j}\|\quad$ |            $\|A\|_{1} = \sum_{i,j}\|A_{i,j}\|$             |
...
\begin{align*}
    \|A\|_{\text{max}}^{\dagger}
        &= \|A\|_1 = \|vec(A)\|_1\\
        &\leq (mn)^{1/1 - 1/2}\|vec(A)\|_2 && \text{(from $(1)$, with $p'=1$ and $q'=2$)}\\
...
\begin{align*}
    \|A\|_{2\to 2}^{\dagger}
        &= \|A\|_{nuc} = \|\sigma(A)\|_1\\
        &\leq (\min\{m,n\})^{1/1 - 1/2}\|\sigma(A)\|_{2} && \text{(from $(1)$, with $p'=1$ and $q'=2$)}\\
...
> $$
\begin{align*}
    \|vec(A)\|_2 &\leq (mn)^{1/2 - 0}\|vec(A)\|_\infty && \text{(from $(1)$, with $p'=2$ and $q'=\infty$)}\\
...
\begin{align*}
    \|\nabla f(X) - \nabla f(X')\|_\text{max}^{\dagger}
        &\leq \sqrt{mn}\|\nabla f(X) - \nabla f(X')\|_F && \text{(from $(2)$)}\\
        &\leq \hat{L}\sqrt{mn}\|X - X'\|_F && \text{(from $(A4)$)}\\
    \|\nabla f(X) - \nabla f(X')\|_\text{max}^{\dagger}
        &\leq \hat{L}mn\|X - X'\|_\text{max} && \text{(from $(4)$)}\tag{5}\\
\end{align*}
$$
...
> $$
\begin{align*}
    \|\sigma(X)\|_2 &\leq (\min\{m,n\})^{1/2 - 0}\|\sigma(X)\|_\infty && \text{(from $(1)$, with $p'=2$ and $q'=\infty$)}\\
...
\begin{align*}
    \|\nabla f(X) - \nabla f(X')\|_{2\to 2}^{\dagger}
        &\leq \sqrt{\min\{m,n\}}\|\nabla f(X) - \nabla f(X')\|_F && \text{(from $(3)$)}\\
        &\leq \hat{L}\sqrt{\min\{m,n\}}\|X - X'\|_F && \text{(from $(A4)$)}\\
    \|\nabla f(X) - \nabla f(X')\|_{2\to 2}^{\dagger}
        &\leq \hat{L}\min\{m,n\}\|X - X'\|_{2\to 2} && \text{(from $(6)$)}\tag{7}\\
\end{align*}
$$
...
@misc{cesista2025tro,
  ...
  month = {March},
  day = {24},
  url = {https://leloykun.github.io/ponder/napkin-math-trust-region-opt/},
}
```
...
2. Jeremy Bernstein, Laker Newhouse (2024). Old Optimizer, New Norm: An Anthology. URL https://arxiv.org/abs/2409.20325
...
6. Cutkosky, A., & Mehta, H. (2020). Momentum improves normalized SGD. In Proceedings of the 37th International Conference on Machine Learning (PMLR Vol. 119, pp. 2260-2268). https://proceedings.mlr.press/v119/cutkosky20b.html
